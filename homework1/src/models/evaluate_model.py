...
def evaluate_model(predicted_labels: np.ndarray, true_labels: pd.Series):
    return {'accuracy': accuracy_score(predicted_labels, true_labels),
            'f1_score': f1_score(predicted_labels, true_labels)}


def serialize_metric(predicted_labels: np.ndarray, true_labels: pd.Series, output_path):
